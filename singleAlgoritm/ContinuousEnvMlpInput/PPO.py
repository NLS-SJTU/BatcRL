...
class ReplayBuffer:
    ...
    def sample_all(self, device):
        return (
            ...
            torch.as_tensor(self.other_buffer[:self.index, 3], device=device),  # noise
        )
...
class ActorPPO(nn.Module):
    def __init__(self, state_dim: int, action_dim: int, mid_dim=512):
        ...
        self.log_sqrt_2pi = torch.log(torch.sqrt(2 * np.pi))
    ...
    def get_action_logprob_entropy(self, states, actions):
        ...
        '''
        delta = 0.5 * ((actions - action_avg) / action_std) ** 2
        logprob = -(delta + action_std.exp() + self.log_sqrt_2pi).sum(1)
...
class PPOAgent:
    ...
    def update(self):
        with torch.no_grad():
            states, actions, reward, mask, noise = self.replay_buffer.sample_all(self.net_device)
